...
- `worker` has **no ports**, so the flattened listener map and the filtered endpoint map simply contain nothing for
  it — no special-casing anywhere.
...
Expected: 15 resources — 3 services, 3 listeners (`api-443`, `api-8443`, `web-443`), 2 public endpoints, 3 runbooks,
3 deploy ids, 1 datastore.
...
In a real pipeline this root has its own role, its own state bucket in a separate account, and a required approval
before apply.
