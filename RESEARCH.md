...
| Kernel role, one VM at a time | `./vms/up.sh <vm>`, its own playbook, then `tests/kernel-reboot.sh <vm>` | all four, booted and torn down one at a time on a 16GB laptop. `kvm-ubuntu` 3 checks, `kvm-suse` 4, `kvm-amazon` 3 (phases 1-3), `kvm-rhel` 8 including the kernel upgrade 5.14.0-427 (9.4) to 5.14.0-687 (9.8) with every managed argument inherited. Each playbook also printed its own family's artifact: the BLS entry plus `GRUB_CMDLINE_LINUX`, `/etc/default/grub.d` in sort order plus the generated `grub.cfg` line, the generator's output file with its mtime, and both Amazon Linux grub keys with `GRUB_CMDLINE_LINUX` reported **absent** |
...
| A35 | The per-distribution playbooks each end by printing their family's artifact, and the Ubuntu one reported **"no generated entry found"** about a `/boot/grub/grub.cfg` that had the tuned boot line in it twice. The pattern was `grep -E '^\s+linux\s+/boot/vmlinuz'`: POSIX extended regular expressions have **no `\s`** - that is a PCRE escape - and the path in a generated grub entry is `/vmlinuz-...`, not `/boot/vmlinuz-...` | the first run of `kernel-ubuntu.yml` against a real VM, where the answer was known to be wrong | `[[:space:]]` and no path prefix. Same class as A33: the check did not fail, it reported the *absence* of something that was there, which is the failure mode that survives review |
| A36 | `SKIP_UPGRADE=1 tests/kernel-reboot.sh kvm-ubuntu` printed the full success message - *"Boot arguments survive a reboot, and a re-apply puts them on a newly installed kernel"* - about a run in which **phase 4 never executed** | running phases 1-3 on a single host while adding the one-at-a-time path | the summary is now conditional and says explicitly that a skipped phase 4 means the run says nothing about a kernel upgrade. A33 again in a third form: a test that claims a result it did not produce |
| A37 | `./vms/up.sh kvm-rhell` (a typo) booted nothing, printed nothing and **exited 0**, which is indistinguishable from "it was already running" - the failure mode the one-at-a-time workflow makes easy to hit, because the name is typed every time instead of never | deliberately typing a wrong name at the new interface | both `up.sh` and `down.sh` validate the names against the known set before doing anything and exit 2 naming the valid ones |

| A38 | `./vms/up.sh kvm-amazon` failed with *"Could not open backing image"* from `qemu-img`, about an image the script had just tried and failed to download. Two bugs: the Amazon Linux URL was a **dated filename under `.../os-images/latest/`**, which is two incompatible ideas in one URL - `latest` moves, the filename does not - and it started returning 403 two days after it was written; and a failed `curl` fell through to the next step, so the error named the wrong layer entirely | re-running the one-at-a-time path on a different day from the day it was written | the URL is pinned to the archived `2023.4.20240401.1` directory, which is also what the comment above it already claimed the image was; and a download failure now stops with the URL that failed. Every image in the table is pinned to an archived version, which is the only way "deliberately old" can be true |
...
- On the **container** rung the kernel role's boot-argument layer is verified by its artifacts only. A container has no bootloader, so the generator step reports that it could not run and the arguments are staged — correct in an image build, and the reason the VM rung exists: `kernel_reboot_ok: true` and the post-reboot confirmation path *are* executed there, by `tests/kernel-reboot.sh`, on four real kernels and on one at a time. What is still not executed anywhere is the same role against **physical** hardware, where firmware settings and a real bootloader disk layout are in play.
